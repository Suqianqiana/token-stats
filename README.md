...
- **毛玻璃模式（Frost）**：第二种材质，与"液态玻璃"并列，可与深浅色自由叠加。
  - **整块磨砂玻璃底板**：竖向宽渐变 + 一层极淡冷蓝薄雾 + 顶部柔和内高光 + 底部内阴影 + 1px 细描边
    （刻意**不用**液态玻璃那套菲涅尔切角与锐利镜面光弧 —— 毛玻璃要的是"糊"，光在面上散开而不是在边上炸开）。
  - **卡片退化为细线分区**：页面里的"卡片"不再铺底、不再描边，只在顶部画 **1px 细线**；
    横向并排的一组（四张指标卡、商汤两张积分池卡）不各画线。列表同时**关掉斑马纹**。
  - **浮层仍然实体**：自绘弹窗与主板照旧铺磨砂底，否则会"隐形"在玻璃上。
  - 入口：右键菜单「🫧 毛玻璃模式」、悬浮球右键菜单，或侧栏底部开关；选择会**落盘记忆**。
- **单一实例**：重复启动不会起第二个常驻实例。
- **一键重启 / 开机自启**：见 §7。
...
| 回归 | `test_switch_race.py` | offscreen 回归测试，**当前 326 项断言**，需全绿 |
...
切页 race 防护、同步数学、cURL 解析（bash / cmd）、指纹与零指纹探测、真实接口 fixture、单列渲染、窗口位置统一、代理 10061 直连重试、多机数据源（导出 / 导入 / 合并 / 覆盖 / 命名）、导入语义（新增 vs 覆盖）、视觉基座（按钮工厂 / 自绘弹窗 / 导航自绘 / 主题联动 / 排行条）、暗色输入框、内联改名、骨架占位行、商汤页池卡常驻与占位满额、按机统计双栏（环形占比口径 / 今日·总量切换 / `metric` 命名守门）、毛玻璃模式（像素级：分区不铺底 / 浮层仍实体 / 开关落盘） —— **326 项断言，需全过**。
...
├── test_switch_race.py     # offscreen 回归测试（326 项断言）
...
5. **变更记录**：
   - **2026-09-23 第60轮**：新增**毛玻璃模式（Frost）**——`theme_state["frost"]` + `paint_frost_surface()` 磨砂材质；`paint_pod()` 加 `role`（分区不铺底 / 浮层仍实体）；列表关斑马纹；`GlassPodFrame(rule=)` 控制分组卡是否画分区线；右键菜单与侧栏新增开关。
